...
// Dynamic imports keep the app working even if the SDK isn't installed yet
...
export function useGoodDollarSDK() {
  ...
  // identitySDKFn is the useIdentitySDK hook — must be called unconditionally
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to fetch entitlement"),
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return {
    ...
    isReady:
      !!claimSDK && !!address && !!publicClient && !!walletClient,
  };
}